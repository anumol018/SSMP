...
#include<stdlib.h>
void main() {
FILE *intermediate, *optab, *symtab, *output, *programlength,*objectcode;
...
programlength = fopen("programlength.txt", "r");
if (!optab || !intermediate || !symtab || !output || !programlength) {//to check whether there is any error in opening the file
...
fscanf(programlength, "%d", &programLength);
